...
try:
  ...
  try:
    ...
    create_image = config['command']['create_image']
    create_variation = config['command']['create_variation']
    create_edit = config['command']['create_edit']
...
l_create_image = len(create_image)
l_create_variation = len(create_variation)
l_create_edit = len(create_edit)
...
# Initialize the DALLE_API instance
dalle_api = DalleApi(api_key=open_ai_token)
...
async def send_image_data_as_file(channel, data, filename):
    data.seek(0)
...
async def on_message(message):
  ...
  if msg.startswith('.' + create_image + ' '):
        prompt = msg[l_create_image + 2:]
        response = dalle_api.create_image(prompt)
        ...
        if "error" in response:
            ...
            await send_image_data_as_file(message.channel, image_data, "created_image.png")



  # create_variation komutunu ekleyin
  if msg.startswith('.' + create_variation):
        if len(message.attachments) > 0:
            ...
            response = dalle_api.create_variation(image_data, n=1, size="1024x1024")

            if "error" in response:
                await message.channel.send(response["error"])
            else:
                variation_url = response['data'][0]['url']
                variation_data = await download_image(variation_url)
                await send_image_data_as_file(message.channel, variation_data, "variation.png")
        else:
            await message.channel.send("Please attach an image file to your message.")

  if msg.startswith('.' + create_edit + ' '):
        if len(message.attachments) > 0:
            image_url = message.attachments[0].url
            edit = msg[l_create_image + 2:]
            image_data = await download_image(image_url)
            ...
            response = dalle_api.create_edit(image_data, edit)

            if "error" in response:
                await message.channel.send(response["error"])
            else:
                edited_url = response['data'][0]['url']
                edited_data = await download_image(edited_url)
                await send_image_data_as_file(message.channel, edited_data, "edited.png")
        else:
            await message.channel.send("Please attach an image file to your message.")
